...
function parseQuestionRow(question, options, correctAnswersRaw, metadata) {
    ...
    // auto-detect correct from [x] inside options if no explicit key column or if it's empty
    if (correctIndices === null || (typeof correctAnswersRaw === 'string' && !correctAnswersRaw.trim())) {
        const auto = [];
        finalOptions.forEach((opt, idx) => {
            if (/[o\-\*]?\s*\[[xX]\]/i.test(opt)) auto.push(idx);
        });
        if (auto.length > 0) {
            // strip the [x] or [ ] from options
            finalOptions = finalOptions.map(opt => opt.replace(/^(?:[o\-\*]?\s*\[[\s_xX]?\])\s*/i, '').trim());
            return {
                text: finalQuestion,
                type: auto.length === 1 ? 'single' : 'multiple',
                options: finalOptions,
                correct: auto.length === 1 ? auto[0] : auto,
                mapel: metadata.subject || 'General', rombel: metadata.class || ''
            };
        }
    }
    ...
    // strip [ ] from options anyway
    const cleanedOptions = finalOptions.map(opt => opt.replace(/^(?:[o\-\*]?\s*\[[\s_xX]?\])\s*/i, '').trim());
    ...
    if (Array.isArray(correctIndices)) {
        return {
            ...
            correct: (correctIndices.length === 1 ? correctIndices[0] : correctIndices.filter(idx => idx < 4)),
            mapel: metadata.subject || 'General', rombel: metadata.class || ''
        };
    }
    return null;
...
function parseSingleTextQuestion(lines, startIndex, metadata, readingText = null) {
    ...
    while (i < lines.length) {
        ...
        if (option) {
            ...
            if (option.isSelected) {
                if (!correctAnswer || /^[A-F]$/i.test(correctAnswer)) {
                    // if no key yet or it's a default 'A', replace with actual marked option
                    if (!Array.isArray(autoCorrectIndices)) autoCorrectIndices = [];
                    // we'll determine the index later
                }
            }
            i++;
        } else {
    ...
    if (finalOptions.length > 0) {
        // Auto-detect correct from [x] markers
        const auto = [];
        finalOptions.forEach((opt, idx) => {
            if (/[o\-\*]?\s*\[[xX]\]/i.test(opt)) auto.push(idx);
        });

        // Clean options from markers
        const cleanedOptions = finalOptions.map(opt => opt.replace(/^(?:[o\-\*]?\s*\[[\s_xX]?\])\s*/i, '').trim());

        // Prioritize explicit correctAnswer from "Kunci: ..." line, but fallback to [x]
        let indices = parseCorrectAnswers(correctAnswer, cleanedOptions);
        if (indices === null && auto.length > 0) indices = auto;
        else if (!correctAnswer || correctAnswer === 'A') {
            // if it was just a default 'A' and we found [x], use the [x]
            if (auto.length > 0) indices = auto;
        }
...
function parseOptionLine(line) {
    ...
    // Check for [ ] or o [ ] format
    const checkMatch = candidate.match(/^(?:[o\-\*]?\s*\[([\s_xX]?)\])\s*(.+)$/i);
    if (checkMatch && checkMatch[2].trim()) {
        const isSelected = !!checkMatch[1].trim();
        return { label: null, text: checkMatch[2].trim(), isSelected };
    }
